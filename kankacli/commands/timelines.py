...
app = typer.Typer(help="List and inspect timelines in your campaign.")
...
def list_timelines(
    ...
    """List all timelines in the campaign."""
    token = require_token()
    cid = _campaign(campaign)
    try:
        with KankaClient(token) as client:
            timelines = list(client.paginate(client.campaign_url(cid, "timelines")))
    except KankaError as e:
        output.error(str(e))
        raise typer.Exit(1)

    if format == Format.json:
        output.print_json(timelines)
    else:
        output.print_table(timelines, LIST_COLUMNS)


@app.command("get")
def get_timeline(
    timeline_id: int = typer.Argument(..., help="Timeline ID."),
    campaign: Optional[int] = typer.Option(None, "--campaign", "-c", help="Campaign ID (overrides default)."),
    format: Format = typer.Option(Format.table, "--format", "-f", help="Output format."),
):
    """Get details for a specific timeline."""
    token = require_token()
    cid = _campaign(campaign)
    try:
        with KankaClient(token) as client:
            result = client.get(client.campaign_url(cid, "timelines", timeline_id))
    except KankaError as e:
        output.error(str(e))
        raise typer.Exit(1)
