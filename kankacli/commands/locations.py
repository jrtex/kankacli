...
def list_locations(
    ...
    try:
        with KankaClient(token) as client:
            locations = list(client.paginate(client.campaign_url(cid, "locations")))
    except KankaError as e:
        output.error(str(e))
        raise typer.Exit(1)

    if format == Format.json:
        output.print_json(locations)
    else:
        output.print_table(locations, LIST_COLUMNS)
...
def get_location(
    ...
    try:
        with KankaClient(token) as client:
            result = client.get(client.campaign_url(cid, "locations", location_id))
    except KankaError as e:
        output.error(str(e))
        raise typer.Exit(1)
...
def add_location(
    ...
    type: Optional[str] = typer.Option(None, "--type", help="Location type (e.g. City, Dungeon, Region)."),
    ...
    try:
        with KankaClient(token) as client:
            result = client.post(client.campaign_url(cid, "locations"), data)
    except KankaError as e:
        output.error(str(e))
        raise typer.Exit(1)

    record = result.get("data", result)
    output.success(f"Location created (id: {record['id']})")
    output.print_record(record, DETAIL_FIELDS)


@app.command("update")
def update_location(
    location_id: int = typer.Argument(..., help="Location ID to update."),
    name: Optional[str] = typer.Option(None, "--name", help="Location name."),
    type: Optional[str] = typer.Option(None, "--type", help="Location type."),
    entry: Optional[str] = typer.Option(None, "--entry", help="Description (supports markdown)."),
    parent_location_id: Optional[int] = typer.Option(None, "--parent-id", help="ID of the parent location."),
    ...
    try:
        with KankaClient(token) as client:
            result = client.patch(client.campaign_url(cid, "locations", location_id), data)
    except KankaError as e:
        output.error(str(e))
        raise typer.Exit(1)

    record = result.get("data", result)
    output.success(f"Location {location_id} updated.")
    output.print_record(record, DETAIL_FIELDS)


@app.command("delete")
def delete_location(
    location_id: int = typer.Argument(..., help="Location ID to delete."),
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation prompt."),
    campaign: Optional[int] = typer.Option(None, "--campaign", "-c", help="Campaign ID (overrides default)."),
):
    """Delete a location. Prompts for confirmation unless --yes is passed."""
    token = require_token()
    cid = _campaign(campaign)

    if not yes:
        typer.confirm(f"Delete location {location_id}? This cannot be undone.", abort=True)

    try:
        with KankaClient(token) as client:
            client.delete(client.campaign_url(cid, "locations", location_id))
    except KankaError as e:
        output.error(str(e))
        raise typer.Exit(1)

    output.success(f"Location {location_id} deleted.")
